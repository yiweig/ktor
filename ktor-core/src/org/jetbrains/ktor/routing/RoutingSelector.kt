...
data class RouteSelectorEvaluation(val succeeded: Boolean, val values: ValuesMap = ValuesMap(), val segmentIncrement: Int = 0)
...
data class AndRoutingSelector(val first: RoutingSelector, val second: RoutingSelector) : RoutingSelector() {
    override fun evaluate(context: RoutingResolveContext, index: Int): RouteSelectorEvaluation {
        ...
        if (!result2.succeeded)
            return result2
        val resultValues = ValuesMap()
        resultValues.appendAll(result1.values)
        resultValues.appendAll(result2.values)
